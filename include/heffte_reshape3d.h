...
/*!
 * \ingroup hefftereshape
 * \brief Reshape algorithm based on the persisten MPI_Alltoall() method.
 *
 * The communication plan for the reshape requires complex initialization,
 * which is put outside of the class into a factory method.
 *
 ...
 */
template<typename location_tag, template<typename device> class packer, typename index>
class reshape3d_alltoallp : public reshape3d_base<index>, public backend::device_instance<location_tag>{
public:
    //! \brief Destructor, frees the comm generated by the constructor.
    ~reshape3d_alltoallp(){
	if (request_is_initialized){
    ...
    //! \brief Factory method, use to construct instances of the class.
    template<typename b, template<typename d> class p, typename i> friend std::unique_ptr<reshape3d_alltoallp<b, p, i>>
    make_reshape3d_alltoallp(typename backend::device_instance<b>::stream_type, std::vector<box3d<i>> const&, std::vector<box3d<i>> const&, bool, MPI_Comm const);
    ...
    //! \brief Templated reshape3d_alltoallp::apply() algorithm for all scalar types.
    template<typename scalar_type>
    void apply_base(int batch_size, scalar_type const source[], scalar_type destination[], scalar_type workspace[]) const;
...
private:
     ...
     * \brief Private constructor that accepts a set of arrays that have been pre-computed by the factory.
     */
    reshape3d_alltoallp(typename backend::device_instance<location_tag>::stream_type q,
                        int input_size, int output_size, bool gpu_aware, MPI_Comm ccomm,
                        std::vector<pack_plan_3d<index>>&&, std::vector<pack_plan_3d<index>>&&,
                        std::vector<int>&&, std::vector<int>&&, int);

    MPI_Comm const comm;
    mutable char *last_used_send;
    mutable char *last_used_recv;
    mutable int last_batch_size;
    mutable MPI_Request request;
    mutable bool request_is_initialized;
    int const me, nprocs;
    bool const use_gpu_aware;
 ...
 * \param comm the communicator associated with all the boxes
 *
 * \returns unique_ptr containing an instance of the heffte::reshape3d_alltoallp
 *
 * Note: the input and output boxes associated with this rank are located at position
 * mpi::comm_rank() in the respective lists.
 */
template<typename location_tag, template<typename device> class packer = direct_packer, typename index>
std::unique_ptr<reshape3d_alltoallp<location_tag, packer, index>>
...
/*!
 * \ingroup hefftereshape
 * \brief Reshape algorithm based on the persistent MPI_Alltoallv() method.
 *
 * The communication plan for the reshape requires complex initialization,
 * which is put outside of the class into a factory method.
 * An instance of the class can be created only via the factory method
 * heffte::make_reshape3d_alltoallvp()
 * which allows for stronger const correctness and reduces memory footprint.
 *
 * \tparam location_tag is the location of the input/output buffers, tag::cpu or tag::gpu
 * \tparam packer the packer algorithms to use in arranging the sub-boxes into the global send/recv buffer,
 ...
 */
template<typename location_tag, template<typename device> class packer, typename index>
class reshape3d_alltoallvp : public reshape3d_base<index>, public backend::device_instance<location_tag>{
public:
    //! \brief Destructor, frees the comm generated by the constructor.
    ~reshape3d_alltoallvp(){
        if (request_is_initialized){
    ...
    //! \brief Factory method, use to construct instances of the class.
    template<typename b, template<typename d> class p, typename i> friend std::unique_ptr<reshape3d_alltoallvp<b, p, i>>
    make_reshape3d_alltoallvp(typename backend::device_instance<b>::stream_type, std::vector<box3d<i>> const&, std::vector<box3d<i>> const&, bool, MPI_Comm const);
...
private:
     ...
     * \brief Private constructor that accepts a set of arrays that have been pre-computed by the factory.
     */
    reshape3d_alltoallvp(typename backend::device_instance<location_tag>::stream_type q,
                        int input_size, int output_size,
                        bool gpu_aware, MPI_Comm new_comm, std::vector<int> const &pgroup,
                        std::vector<int> &&send_offset, std::vector<int> &&send_size, std::vector<int> const &send_proc,
                        std::vector<int> &&recv_offset, std::vector<int> &&recv_size, std::vector<int> const &recv_proc,
    ...
    iotripple const send, recv;
    mutable iotripple batched_send, batched_recv; // send and recv, scaled by the batch_size
};
    ...
    if (match(input_boxes, output_boxes)){
        ...
        if (options.algorithm == reshape_algorithm::alltoallv){
            ...
            if (input_boxes[0].ordered_same_as(output_boxes[0])){
                ...
                return make_reshape3d_alltoallvp<location_tag, transpose_packer, index>(stream, input_boxes, output_boxes,
                                                                                       options.use_gpu_aware, comm);
            }
            ...
            if (input_boxes[0].ordered_same_as(output_boxes[0])){
                ...
                return make_reshape3d_alltoallp<location_tag, transpose_packer, index>(stream, input_boxes, output_boxes,
                                                                                      options.use_gpu_aware, comm);
            }
        }else{
            if (input_boxes[0].ordered_same_as(output_boxes[0])){
