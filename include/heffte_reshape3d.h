...
public:
    //! \brief Destructor, frees the comm generated by the constructor and any initialized MPI requests.
    ~reshape3d_alltoall(){
    ...
    make_reshape3d_alltoall(typename backend::device_instance<b>::stream_type, std::vector<box3d<i>> const&, std::vector<box3d<i>> const&, reshape_algorithm, bool, MPI_Comm const);
...
make_reshape3d_alltoall(typename backend::device_instance<location_tag>::stream_type q,
                        ...
                        reshape_algorithm algorithm, bool uses_gpu_aware, MPI_Comm const comm);
...
public:
    ...
    make_reshape3d_alltoallv(typename backend::device_instance<b>::stream_type, std::vector<box3d<i>> const&, std::vector<box3d<i>> const&, reshape_algorithm, bool, MPI_Comm const);
...
private:
    ...
    reshape3d_alltoallv(typename backend::device_instance<location_tag>::stream_type q,
                        int input_size, int output_size, reshape_algorithm alg,
    ...
    mutable iotripple batched_send, batched_recv; // send and recv, scaled by the batch_size (for persistent)
};
...
public:
    ...
    ~reshape3d_pointtopoint(){
        if (persistent_reqs_initialized) {
        ...
        if (persistent_isends_initialized) {
...
private:
    ...
    mutable bool persistent_reqs_initialized;
    mutable bool persistent_isends_initialized;
...
std::unique_ptr<reshape3d_base<index>> make_reshape3d(typename backend::device_instance<typename backend::buffer_traits<backend_tag>::location>::stream_type stream,
    ...
    if (match(input_boxes, output_boxes)){
        ...
        if (options.algorithm == reshape_algorithm::alltoallv || options.algorithm == reshape_algorithm::alltoallv_p){
            if (input_boxes[0].ordered_same_as(output_boxes[0])){
        ...
        }else if (options.algorithm == reshape_algorithm::alltoall || options.algorithm == reshape_algorithm::alltoall_p){
            if (input_boxes[0].ordered_same_as(output_boxes[0])){
